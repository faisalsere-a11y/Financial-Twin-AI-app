...
function SubjectInvestmentSimulator({ activeProfile }: { activeProfile: ReturnType<typeof useFinancialProfile> }) {
  ...
  return (
    <div className="mx-auto max-w-[1320px]">
      ...
      <div className="mb-6 rounded-2xl border border-border bg-card/70 p-4 text-sm text-muted-foreground">
      ...
      <div className="grid gap-6 xl:grid-cols-[22rem_1fr] xl:items-start">
        <Card className="border-primary/20 bg-primary/5 xl:sticky xl:top-24">
          ...
          <CardContent className="grid gap-5 p-5 sm:p-6">
            ...
            <div className="rounded-xl border border-border bg-background/60 p-4">
              <div className="flex items-start gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon className="size-4" aria-hidden="true" /></span><div><p className="font-black">{asset}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{preset.description}</p></div></div>
        ...
        <div className="grid gap-6">
          <div aria-live="polite" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ["Expected value", formatCurrency(projection.futureValue, profile.currency)],
              ["Modeled gain", formatCurrency(projection.gain, profile.currency)],
              ["P10 lower outcome", formatCurrency(monteCarlo.p10, profile.currency)],
              ["P50 median", formatCurrency(monteCarlo.median, profile.currency)]
            ].map(([label, value]) => <Card key={label}><CardContent className="p-5"><p className="text-xs font-bold uppercase tracking-[0.1em] text-muted-foreground">{label}</p><p className="mt-2 text-2xl font-black tabular-nums">{value}</p></CardContent></Card>)}
          </div>
            ...
            <div className="h-80" aria-hidden="true">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={projection.points}>
                  ...
                  <Area type="monotone" dataKey="optimistic" stroke={chartTheme.after} fill={chartTheme.after} fillOpacity={0.08} />
                  <Area type="monotone" dataKey="expected" stroke={chartTheme.current} fill={chartTheme.current} fillOpacity={0.1} />
                  <Area type="monotone" dataKey="conservative" stroke={chartTheme.comparison} fill={chartTheme.comparison} fillOpacity={0.06} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </ChartFrame>
            ...
            <div className="h-80" aria-hidden="true">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={monteCarlo.paths}>
                  ...
                  <YAxis stroke={chartTheme.axis} tickLine={false} axisLine={false} />
                  <Tooltip contentStyle={chartTooltipStyle} />
                  <Line type="monotone" dataKey="optimistic" stroke={chartTheme.after} strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="expected" stroke={chartTheme.current} strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="pessimistic" stroke={chartTheme.comparison} strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </ChartFrame>
          ...
          <Card>
            ...
            <CardContent className="grid gap-3 text-sm leading-6 text-muted-foreground sm:grid-cols-3">
              <p><strong className="text-foreground">P10</strong> means 10% of seeded runs ended at or below {formatCurrency(monteCarlo.p10, profile.currency)}.</p>
              <p><strong className="text-foreground">P50</strong> is the median seeded result: {formatCurrency(monteCarlo.median, profile.currency)}.</p>
              <p><strong className="text-foreground">P90</strong> means 90% ended at or below {formatCurrency(monteCarlo.p90, profile.currency)}.</p>
              <p className="flex gap-2 sm:col-span-3"><ShieldCheck className="mt-1 size-4 shrink-0 text-primary" aria-hidden="true" />Returns are not guaranteed. These ranges are educational outputs from fixed assumptions, not market predictions or investment advice.</p>
            </CardContent>
          </Card>
        </div>
      </div>
