  ...
  return (
    <section aria-label="Interactive financial decision preview" className="premium-ring relative overflow-hidden rounded-[2rem] border border-border bg-card/90 p-4 shadow-glass-strong sm:p-6">
      ...
      <div className="relative">
        ...
        <div role="tablist" aria-label="Choose a financial decision" className="mt-5 grid gap-2 sm:grid-cols-3">
          {landingScenarioOptions.map((option) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "min-h-12 rounded-xl border px-3 py-2 text-left text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                  selected ? "border-primary/40 bg-primary/10 text-primary" : "border-border bg-muted/40 text-muted-foreground hover:bg-muted"
                )}
              >
                {option.label}
              </button>
        ...
        >
          ...
          <div className="rounded-2xl border border-border bg-background/35 p-4">
            ...
            <svg viewBox="0 0 560 190" role="img" aria-label={preview.overview.cashFlowSummary} className="h-44 w-full overflow-visible">
              ...
              <polyline points={currentPoints} fill="none" stroke="hsl(var(--chart-1))" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
              <polyline points={afterPoints} fill="none" stroke="hsl(var(--chart-2))" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
